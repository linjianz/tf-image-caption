...
Creat Time: 2017-10-04 18:40:12
Program: 
Description: 
"""
...
flags = tf.app.flags
flags.DEFINE_string('dir0', '20171005_merged', 'directory name to save the model/log')
flags.DEFINE_string('net_name', 'image_caption_ai/', 'project name')
flags.DEFINE_integer('batch_size', 256, 'batch size')
flags.DEFINE_integer('epoch_max', 50, 'max epoch number')
flags.DEFINE_integer('epoch_save', 5, 'save model every # epoch')
...
def load_data_all():
    with open('data/ai/index2token.pkl', 'r') as f:
        index2token = pickle.load(f)
    with open('data/ai/preprocessed_captions_merged.pkl', 'r') as f:
        captions, caption_id2sentence, caption_id2image_id = pickle.load(f)
    with open('data/ai/img_vector_merged.pkl', 'r') as f:
        image_id2feature = pickle.load(f)
...
def main(_):
    index2token, captions, caption_id2sentence, caption_id2image_id, image_id2feature = load_data_train()
